...
export default function Navbar({ index, ext }: props) {
    ...
    return <div className="w-full h-[10vh] sticky top-0 md:py-4 bg-primary-light-200 dark:bg-primary-dark-800 bg-opacity-75 dark:bg-opacity-[60%] dark:backdrop-saturate-[120%] backdrop-blur-sm shadow-xl z-30">
        ...
        {!ext && <div className={`absolute top-[100%] lg:hidden left-0 p-4 bg-primary-light-200 dark:bg-primary-dark-800 z-40 bg-opacity-75 dark:bg-opacity-[60%] dark:backdrop-saturate-[120%] backdrop-blur-sm shadow-xl transition ${navOpen ? "translate-x-0 opacity-100" : "-translate-x-[100%] opacity-0"}`}>
            {navigation.map(nav => nav.localUrl ? <LocalLink className={`p-4 font-semibold text-lg hover:scale-105 transition cursor-pointer ${hoverTextColors[index]}`} target={nav.target} key={nav.name}>
                {nav.name}
            </LocalLink> : <Link to={nav.target.toString()} className={`p-4 font-semibold text-lg hover:scale-105 transition cursor-pointer ${hoverTextColors[index]}`}>
                {nav.name}
